...
def fill_nodata_nearest(image: np.ndarray, nodata: np.ndarray) -> np.ndarray:
    """fill masked pixels with the nearest valid value before wavelet filtering."""
    if nodata is None or not np.any(nodata):
    ...
    # nearest-neighbor fill keeps the large black rotated borders from dominating
    # the wavelet and fourier coefficients
    nearest_indices = distance_transform_edt(
        nodata,
        return_distances=False,
        return_indices=True,
    )
    filled = image.astype(np.float64, copy=True)
    filled[nodata] = image[tuple(nearest_indices[:, nodata])]
    return filled


def muench_complete_modified(image: np.ndarray, nodata: np.ndarray, dec_num_v: int,
                               sigma_v: float, dec_num_h: int,
                               sigma_h: float, intermediates_dir_path: str,
                               wavelet: str = "db15") -> Tuple[np.ndarray, Dict]:
    """paper-faithful wavelet-fft destriping core."""
    if image.ndim != 2:
        raise ValueError("muench core expects one 2d band")
    if image.dtype != np.uint8:
        raise TypeError("muench core expects uint8 input")
    if dec_num_v < 0 or dec_num_h < 0:
        raise ValueError("decomposition levels must be non-negative")
    if dec_num_v > 0 and sigma_v <= 0:
        raise ValueError("sigma_v must be greater than 0")
    if dec_num_h > 0 and sigma_h <= 0:
        raise ValueError("sigma_h must be greater than 0")

    original_h, original_w = image.shape

    # rgb values outside the rotated footprint are already reflect-filled during rotation.
    # do not replace them with zeros before the wavelet transform.
    ima = image.astype(np.float64, copy=True)
    ...
    max_level = pywt.dwtn_max_level(ima.shape, wave)
    if dec_num > max_level:
        raise ValueError(
            f"dec_num={dec_num} is too high for image shape {ima.shape} "
            f"with wavelet {wavelet}; maximum is {max_level}"
        )

    ch_levels, cv_levels, cd_levels = [], [], []
    approximation_shapes = []

    # matches fig. 5 of the paper: recursively decompose only the approximation band.
    for _ in range(dec_num):
        ima, (ch, cv, cd) = pywt.dwt2(ima, wavelet, mode="symmetric")
        ch_levels.append(ch)
        cv_levels.append(cv)
        cd_levels.append(cd)
        approximation_shapes.append(ima.shape)

    # vertical stripes: filter cv with a column-wise 1d fft (axis 0), exactly as in the paper.
    for level in range(dec_num_v):
        cv = cv_levels[level]
        fcv = fftshift(fft(cv, axis=0), axes=0)
        rows = fcv.shape[0]
        k = np.arange(-rows // 2, -rows // 2 + rows, dtype=np.float64)
        damp = 1.0 - np.exp(-(k * k) / (2.0 * sigma_v * sigma_v))
        cv_levels[level] = np.real(
            ifft(ifftshift(fcv * damp[:, None], axes=0), axis=0)
        )

    # optional horizontal-stripe counterpart.
    for level in range(dec_num_h):
        ch = ch_levels[level]
        fch = fftshift(fft(ch, axis=1), axes=1)
        cols = fch.shape[1]
        k = np.arange(-cols // 2, -cols // 2 + cols, dtype=np.float64)
        damp = 1.0 - np.exp(-(k * k) / (2.0 * sigma_h * sigma_h))
        ch_levels[level] = np.real(
            ifft(ifftshift(fch * damp[None, :], axes=1), axis=1)
        )

    # matches the matlab reconstruction loop in fig. 5.
    nima = ima
    for level in range(dec_num - 1, -1, -1):
        target = ch_levels[level].shape
        nima = nima[:target[0], :target[1]]
        nima = pywt.idwt2(
            (nima, (ch_levels[level], cv_levels[level], cd_levels[level])),
            wavelet,
            mode="symmetric",
        )

    nima = nima[:original_h, :original_w]
    nima = np.nan_to_num(nima, nan=0.0, posinf=255.0, neginf=0.0)
    nima = np.clip(np.rint(nima), 0, 255).astype(np.uint8)

    # restore transparency only after filtering.
    if nodata is not None:
        nima[nodata[:original_h, :original_w]] = 0
...
def rotate_rgb_geotiff(input_path, output_path, angle=14.3):
    ...
    with rasterio.open(input_path) as src:
        ...
        if crs is None:
            print("No CRS found. Rotating in image space instead.")
            ...
            dst_width = int(np.ceil(height * sin_a + width * cos_a))
            dst_height = int(np.ceil(height * cos_a + width * sin_a))

            matrix[0, 2] += dst_width / 2.0 - center[0]
            matrix[1, 2] += dst_height / 2.0 - center[1]

            # rotate rgb with reflected borders so wavelet/fft never sees a hard black edge.
            # rotate alpha separately with a zero border so the final footprint stays transparent.
            ...
            rotated = np.dstack((rotated_rgb, rotated_alpha))
            profile.update(
                height=dst_height,
                width=dst_width,
            ...
            with rasterio.open(output_path, "w", **profile) as dst:
                dst.write(np.transpose(rotated, (2, 0, 1)))
            return
...
def process_rgba(input_path: str, output_path: str,
            intermediates_dir: str, enhance_contrast: bool = False, dec_num_v=5, dec_num_h=5, sigma_v=2.4, sigma_h=2.4, save_statistics=True, min_max_rgba=(), wavelet='db15', rotation_angle=14.3) -> None:
    """
    ...
    Args:
        ...
        wavelet: Wavelet name passed to PyWavelets, such as db4 or db38
    """
    # Read image
    ...
    for band_idx in range(3):
        ...
        destriped_band, statistics = muench_complete_modified(
            band, nodata, dec_num_v, sigma_v, dec_num_h, sigma_h, intermediates_dir, wavelet=wavelet
        )       
        processed_bands.append(destriped_band)
        all_statistics.append(statistics)
    # the alpha mask is geometry, not image data, so leave it unchanged
    alpha_statistics = None
    ...
    shutil.copy(processed_output_path, output_path)
    ...
    if save_statistics:
        ...
        if alpha_statistics is not None:
