...
def muench_complete_modified_test(image: np.ndarray, nodata, nodata_stripes_flagged: List[np.ndarray], dec_num_v: int, sigma_v: float, dec_num_h: int, sigma_h: float, intermediates_dir_path: str, wavelet: str = 'db25') -> Tuple[np.ndarray, Dict]:
        """
        Remove vertical stripes using Muench et al. (2009) method.
        ...
        nodata_Ch, nodata_Cv, nodata_Cd = nodata_stripes_flagged
        ...
        for i in range(dec_num_v):
            ...
            # Cv_temp = Cv[i].copy()
            damped_cv = np.real(ifft(ifftshift(fCv_damped, axes=0), axis=0))
            Cv[i] = np.where(~nodata_Cv[i], damped_cv, Cv[i])
            Cv_stripes[i] -= np.where(~nodata_Cv[i], damped_cv, Cv[i])
        ...
        for i in range(dec_num_h):
            ...
            # Inverse FFT
            damped_ch = np.real(ifft(ifftshift(fCh_damped, axes=1), axis=1))
            Ch[i] = np.where(~nodata_Ch[i], damped_ch, ch)
            Ch_stripes[i] -= np.where(~nodata_Ch[i], damped_ch, ch)
...
def muench_get_nodata_outline(nodata: np.ndarray, dec_num_v: int, dec_num_h: int, intermediates_dir_path: str, wavelet: str = 'db25', threshold_for_nodata_wavelet: float = 10) -> Tuple[np.ndarray, Dict]:
        """
        Remove vertical stripes using Muench et al. (2009) method.
        ...
        Returns:
            Image with vertical stripes removed
        """
        assert nodata.dtype == np.bool
        ima = nodata.astype(np.float64)*255

        dec_num = max(dec_num_v, dec_num_h)

        # Store detail coefficients
        Ch, Cv, Cd = [], [], []
        Ch_flagged, Cv_flagged, Cd_flagged = [], [], []
        ...
        for i in range(dec_num):
            ...
            Ch_flagged.append(ch > threshold_for_nodata_wavelet)
            Cv_flagged.append(cv > threshold_for_nodata_wavelet)
            Cd_flagged.append(cd > threshold_for_nodata_wavelet)

        # Save Cv coefficients as images for each decomposition level
        for i, cv in enumerate(Cv):
            norm_output_path = os.path.join(intermediates_dir_path, f"nodata_Cv_level_{i}_norm.png")
            # Normalize for visualization
            cv_norm = cv.copy()
            cv_norm = cv_norm - np.min(cv_norm)
            if np.max(cv_norm) > 0:
                cv_norm = cv_norm / np.max(cv_norm)
            cv_img = (cv_norm * 255).astype(np.uint8)
            plt.imsave(norm_output_path , cv_img, cmap='gray')
            absolute_output_path = os.path.join(intermediates_dir_path, f"nodata_Cv_level_{i}_absolute.png")
            cv_img = (cv.copy() * 255).astype(np.uint8)
            plt.imsave(absolute_output_path , cv_img, cmap='gray')

        return Ch_flagged, Cv_flagged, Cd_flagged
...
def process_rgba(input_path: str, output_path: str,
    ...
    Ch_flagged, Cv_flagged, Cd_flagged = muench_get_nodata_outline(nodata, dec_num_v, dec_num_h, intermediates_dir)

    no_data_stripes_flagged = [Ch_flagged, Cv_flagged, Cd_flagged]

    # Iterate over bands using the last axis (channels)
    for band_idx in range(3):
        band = rotated_image[:, :, band_idx]
        destriped_band, statistics = muench_complete_modified_test(
            band, nodata, no_data_stripes_flagged, dec_num_v, sigma_v, dec_num_h, sigma_h, intermediates_dir, wavelet='db25'
        )
...
if __name__ == "__main__":
    ...
    geotiff_path = "/Volumes/LaCie/test_destriper/test_data/chlor_a_oceancolor.tif"
    output_path = "/Volumes/LaCie/test_destriper/modified_muench_test_land_mask_1/chlor_a_oceancolor_processed.tif"
    intermediates_dir = "/Volumes/LaCie/test_destriper/modified_muench_tests/modified_muench_test_land_mask_1/intermediates"
    dec_num_v=6
    dec_num_h=0
    sigma_v=2.
    sigma_h=0.
    ...
    geotiff_path = "/Volumes/LaCie/test_destriper/test_data/chlor_a_oceancolor.tif"
    output_path = "/Volumes/LaCie/test_destriper/modified_muench_test_land_mask_2/chlor_a_oceancolor_processed.tif"
    intermediates_dir = "/Volumes/LaCie/test_destriper/modified_muench_tests/modified_muench_test_land_mask_2/intermediates"
    dec_num_v = 9
    dec_num_h = 0
    sigma_v = 2.
    sigma_h = 0.
    ...
    geotiff_path = "/Volumes/LaCie/test_destriper/test_data/chlor_a_oceancolor.tif"
    output_path = "/Volumes/LaCie/test_destriper/modified_muench_test_land_mask_3/chlor_a_oceancolor_processed.tif"
    intermediates_dir = "/Volumes/LaCie/test_destriper/modified_muench_tests/modified_muench_test_land_mask_3/intermediates"
    dec_num_v = 12
    dec_num_h = 0
    sigma_v = 2.
    sigma_h = 0.
    ...
    geotiff_path = "/Volumes/LaCie/test_destriper/test_data/chlor_a_oceancolor.tif"
    output_path = "/Volumes/LaCie/test_destriper/modified_muench_test_land_mask_4/chlor_a_oceancolor_processed.tif"
    intermediates_dir = "/Volumes/LaCie/test_destriper/modified_muench_tests/modified_muench_test_land_mask_4/intermediates"
    dec_num_v = 6
    dec_num_h = 0
    sigma_v = 7.
    sigma_h = 0.
    ...
    geotiff_path = "/Volumes/LaCie/test_destriper/test_data/chlor_a_oceancolor.tif"
    output_path = "/Volumes/LaCie/test_destriper/modified_muench_test_land_mask_5/chlor_a_oceancolor_processed.tif"
    intermediates_dir = "/Volumes/LaCie/test_destriper/modified_muench_tests/modified_muench_test_land_mask_5/intermediates"
    dec_num_v = 9
    dec_num_h = 0
    sigma_v = 7.
    sigma_h = 0
    ...
    geotiff_path = "/Volumes/LaCie/test_destriper/test_data/chlor_a_oceancolor.tif"
    output_path = "/Volumes/LaCie/test_destriper/modified_muench_test_land_mask_6/chlor_a_oceancolor_processed.tif"
    intermediates_dir = "/Volumes/LaCie/test_destriper/modified_muench_tests/modified_muench_test_land_mask_6/intermediates"
    dec_num_v = 12
    dec_num_h = 0
    sigma_v = 7.
    sigma_h = 0
    ...
    geotiff_path = "/Volumes/LaCie/test_destriper/test_data/chlor_a_oceancolor.tif"
    output_path = "/Volumes/LaCie/test_destriper/modified_muench_test_land_mask_7/chlor_a_oceancolor_processed.tif"
    intermediates_dir = "/Volumes/LaCie/test_destriper/modified_muench_tests/modified_muench_test_land_mask_7/intermediates"
    dec_num_v = 6
    dec_num_h = 0
    sigma_v = 12.
    sigma_h = 0.
    ...
    geotiff_path = "/Volumes/LaCie/test_destriper/test_data/chlor_a_oceancolor.tif"
    output_path = "/Volumes/LaCie/test_destriper/modified_muench_test_land_mask_8/chlor_a_oceancolor_processed.tif"
    intermediates_dir = "/Volumes/LaCie/test_destriper/modified_muench_tests/modified_muench_test_land_mask_8/intermediates"
    dec_num_v = 9
    dec_num_h = 0
    sigma_v = 12.
    sigma_h = 0
    ...
    geotiff_path = "/Volumes/LaCie/test_destriper/test_data/chlor_a_oceancolor.tif"
    output_path = "/Volumes/LaCie/test_destriper/modified_muench_test_land_mask_9/chlor_a_oceancolor_processed.tif"
    intermediates_dir = "/Volumes/LaCie/test_destriper/modified_muench_tests/modified_muench_test_land_mask_9/intermediates"
    dec_num_v = 12
    dec_num_h = 0
    sigma_v = 12.
    sigma_h = 0
